...
export const getExperimentAggregations = scopedQuery(
  async function getExperimentAggregations(
    ...
    const { workspaceId, experimentUuids, experimentIds, documentUuid } = params

    const conditions: string[] = ['workspace_id = {workspaceId: UInt64}']
    ...
    if (experimentUuids && experimentUuids.length > 0) {
      ...
      queryParams.experimentUuids = experimentUuids
    }
